...
const Var = require('../virtualizing/Source/enum/Var')

const Datas = Virtual.define({
  name: 'datas',
  class: (base) => {
    return class Datas extends base {
      static build(options) {
        var properties = {};
        for (let modelName in options.models) {
          let model = options.models[modelName];
          properties[model.pluralName] = {
            type: Virtuals.collection.of(model.virtual)
          }
        }
        const newClass = Datas.define({
          name: 'datas',
          properties
        })
        return new newClass({
          source: new Var('datas'),
