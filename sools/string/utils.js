...
var stringUtilities = {
  ...
  */
  processKeyValue: function(source, work, options) {
    options = options || {}
    var key = "";
    var value;
    var inString = false;
      ...
      if (inValue) {
        if (inRegex) {
          ...
          if (escapeChar) {
        	...
        	if(!extremities && ['[','{'].indexOf(c) != -1){
        		extremities = {
        			start:c,
        			end:c == "[" ? ']' : '}'
        		}
        	}
          if (extremities && c == extremities.start) {
            scopeCount++;
          } else if (extremities && c == extremities.end) {
            scopeCount--;
          } else {
        ...
        if ((c == ',' || i == source.length - 2) && !inRegex && !inString && !scopeCount) {
      	...
      	var delimiterString = options.delimiter || ':'
      	var delimiter = stringUtilities.fromTo(source,i,i + delimiterString.length )
        if (delimiter == delimiterString) {        	
          value = "";
          inValue = true;
          i+= delimiterString.length - 1
        } else {
          key += c;
        }
      }
